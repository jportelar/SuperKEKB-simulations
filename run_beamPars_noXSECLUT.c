...
const unsigned int _chi2binMin = 0*_nBins/200.;
...
void fillLUTbinsBeamGas() {
    ...
    cout << "hello" << endl;
    hEgBeamGasLUT->Reset();
    hEgBeamGasLUT_0->Reset();
    hEgBeamGasLUT_21->Reset();
    if(_curZ>7.5 || _curZ<0) {
        for (int k=0;k<hEgBeamGasLUT->GetNbinsX();k++) {
            hEgBeamGasLUT->SetBinContent(k+1,1/((double)hEgBeamGasLUT->GetNbinsX()));
            hEgBeamGasLUT_0->SetBinContent(k+1,1/((double)hEgBeamGasLUT->GetNbinsX()));
            hEgBeamGasLUT_21->SetBinContent(k+1,1/((double)hEgBeamGasLUT->GetNbinsX()));
        }
        return;
    }
    ...
    for (int kph=0;kph<nMax;kph++) {
        ...
        if (xsec_rnd<xsec_calc) {
            ...
            //cout << hEgBeamGasLUT->GetBinContent(hEgBeamGasLUT->FindBin(Eg_meas)) << endl;
        }
        else {
            kph--;
        }
...
double inline functionEval(int ibin,double f1, double f2,double Pz,double PLasCirc,double norm,double x0,double nExp,double f3, double nBGExp, double Z, double f4) {
    ...
    double valueBG = f3*TMath::PoissonI(1,nBGExp)*hEgBeamGasLUT->GetBinContent(ibin+1);//*xsecBGTot(Z,7e9)/xsecBGTot(2.5,7e9)
    //cout << ibin << " " << hEg->GetBinCenter(ibin+1) << " " << valueBG << endl;
    valueBG += f4/*f1*f3*/*TMath::PoissonI(1,nBGExp)*TMath::PoissonI(1,nExp)*(hEgBeamGasLUT_0->GetBinContent(ibin+1)+hEgBeamGasLUT_21->GetBinContent(ibin+1)*Pz*PLasCirc);
    ...
    //cout << ibin << " " << value << " " << valueBG << " " << norm << " " << f3 << " " <<TMath::PoissonI(1,nBGExp) << " " <<hEgBeamGasLUT->GetBinContent(ibin+1) << " " <<xsecBGTot(Z,7e9)/xsecBGTot(2.5,7e9) << endl;
    ...
    if(matchNGen) {
     for (unsigned int k =0;k<nGen_;k++) {
        ...
        for (unsigned int k=0;k<Nturns_;k++) {
            ...
            //if (nBGPhotons_>1)
            //    nBGPhotons_ = 1;
            double Afact=6.1; //has been empirically obtained with mathematica valid up to Z=7.5
            //int nTrials=0;
            //int nKept=0;
...
void fitData(TString filename,TH1D *hFun=NULL,ROOT::Math::Minimizer* min=NULL,bool useNgen=true,bool isNotToy=true,bool isFirstEvent=true,bool doScan=false, bool beamGasOnly=false) {
   ...
   // if(isFirstEvent) {
    ...
    if (beamGasOnly) {
        ...
        //min->SetVariable(9,"energyResol_A",10.,1.); //percents units
        ...
        min->SetVariableLimits(19,1.1,7.5);
        //min->SetVariableLimits(20,0.,100000);
        min->SetVariableLimits(22,0.0001,1000);
    ...
    //const ROOT::Minuit2::MnUserParameterState state = ((ROOT::Minuit2::Minuit2Minimizer*) min)->State();
    //cout << state << endl;
    //cout << " Scale parameters " << state.Value(19) << " " << state.Error(19) << endl;
    ...
    if(!min->IsFixedVariable(19)) {
        double Zbg_[10];
        double chi2_[10];
        unsigned int nstep=10;
        //min->Scan(19,nstep,Zbg_,chi2_,1.0,7.5);
        min->FixVariable(20);
        //min->SetVariableStepSize(19,0.1);
        //min->Minimize();
        //min->ReleaseVariable(20);
    }
    min->Minimize();
    ...
    if ( ((min->Status()>0 ||  maxGCC(min)>0.9) && (min->IsFixedVariable(7) || min->IsFixedVariable(16)))) {
        cout <<"IMPROVE by refitting because of status or GCC>0.9" << endl;
        min->Minimize();
    }
        ...
        cout <<"IMPROVE by refitting because of status or GCC>0.9 for par 0 or 1" << endl;
        min->Hesse();
        min->Minimize();
    }
    //min->Hesse();
    ...
    //double errZLow, errZUp;
    //min->GetMinosError(19,errZLow,errZUp);
    //cout <<"minos errors " << errZLow << " " << errZUp << endl;
...
*/
    ...
    if (isNotToy && doScan) {
        ...
        if( beamGasOnly) {
            ...
            double Zbg_[100];
            double chi2_[100];
            TTree *t = new TTree("scanBeamGasZ","scanBeamGasZ");
            t->Branch("Zbg",Zbg_,"Zbg[100]/D");
            t->Branch("chi2",chi2_,"chi2[100]/D");
            unsigned int nstep=100;
            min->Scan(19,nstep,Zbg_,chi2_,2.3,2.7);
            t->Fill();
            t->Write();
        }
        else {
    ...
    const double *fitresErr2 = min->Errors();
    cout <<fitresErr2[19] << " " << fitresErr2[20] << endl;
...
void toyMC(TString filename,unsigned int nToys,unsigned long seedShift=1000,bool beamGasOnly=false) {
     ...
     //run toy
      for (unsigned int k=0;k<nToys;k++) {
          ...
          fitData(TString("paramsToy.root"),hFun_,min,false,false,(bool) (k==0),false,beamGasOnly);
          t1 = clock();
